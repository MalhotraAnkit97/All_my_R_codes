...
ggplot(data = mpg, aes(x = displ, y = hwy, group=drv)) + 
  geom_point(size=5)+
  geom_smooth( color = "blue", size=3, se=F)
```
...
ggplot(data = mpg, aes(x = displ, y = hwy, color=drv, linetype=drv)) + 
  geom_point(size=2)+
  geom_smooth( color='blue', size=2, se=F)
```
...
|-----------------|------------|----------------|
| `geom_bar()`        |  stat_count()  | the number of cases at each x position   |
| `geom_histogram()`  |  stat_bin()  | divide the x axis into bins and counting the number of observations in each bin.   |
| `geom_density()`    |  stat_density()  | Computes and draws kernel density estimate, which is a smoothed version of the histogram   |
| `geom_point()`      |  identity  |  use the original data  | 
| `geom_smooth()`     |  stat_smooth()  |  Smoothed pattern from the scatterplot  |

Some `geom` function has stat = "identity" as the default. What does that mean?

**INSERT_YOUR_ANSWER**

No data transformation is done, and the original data are used.
...
```{r}
library(gridExtra)
gg=ggplot(data = mpg, mapping=aes(x=drv,fill = fl)) 
...
```{r}
library(gridExtra)
gg=ggplot(data = quakes) 
plot1=gg+geom_histogram(mapping=aes(x=mag, y=..density..), fill='white')+
        geom_density(mapping=aes(x=mag), colour='red')
plot2=gg+geom_boxplot(mapping=aes(x=mag)) 
plot3=gg+stat_ecdf(mapping=aes(x=mag))
#plot4=ggplot(data = quakes, mapping=aes(sample=mag))+stat_qq()+stat_qq_line()
plot4=ggplot(data = quakes, mapping=aes(sample=mag))+stat_qq()+stat_qq_line(colour='blue')
grid.arrange(plot1, plot2, plot3, plot4, ncol=2)
```
...
xlim <- c(min(quakes$long)/2, max(quakes$long)+10)
ylim <- c(min(quakes$lat)-10, max(quakes$lat)+10)

 bbox=make_bbox(xlim, ylim)
# bbox <- make_bbox(lon = quakes$long, lat = quakes$lat, f = .1)

bc_big <- get_map(location = bbox, source='google', maptype = "terrain")
...
map <- ggplot() + coord_map() + geom_polygon(world, mapping=aes(x = long, y = lat, group = group), fill = "white", color = "black") 

map + 
  ylim(min(quakes$lat)-10, max(quakes$lat)+10) + 
  xlim(min(quakes$long)/2, max(quakes$long)+10) + 
  geom_point(quakes, mapping = aes(x = long, y = lat, size = mag, color = depth))
