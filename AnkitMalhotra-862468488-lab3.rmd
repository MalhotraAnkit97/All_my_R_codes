...
```{r}
ggplot(data = mpg, 
       mapping = aes(x = cty, y = displ, col = drv) # global mapping
       ) +
  geom_point(mapping = aes(size = cyl)) +
  geom_smooth(method = "lm") # local mapping for the layer # For regression
```


What you can conclude from your figure?

**We can conclude by the scatterplot that the variables follow a negative correlation**
...
**If we Facet on a continuous variable, the data given will not make sense and look absurd, as there are SO many numerical points, and we can only apply Facet on a Categorical variable. So we need to change the continous variable to discrete to make more sense of data.**
...
**In the given R code using ggplot2, the . serves as a shortcut that essentially says "include all other variables" when creating a grid of facets. Specifically, the line facet_grid(. ~ class) indicates the desire to generate a grid of individual plots (facets) for each unique category (level) within the "class" variable.**
